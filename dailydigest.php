...
function createDigest(&$analytics) {
  try {
    ...
    if (isset($profileId)) {
      ...
      $message = "*DAILY DIGEST*\n Yesterday (" . $yesterday->format('l') . "), we had <https://www.google.com/analytics/web/?hl=en#report/visitors-overview/" . GOOGLE_ANALYTICS_WEB_ID . "/%3F_u.date00%3D" . $yesterday->format('Ymd') . "%26_u.date01%3D" . $yesterday->format('Ymd') . "%26overview-graphOptions.selected%3Danalytics.nthHour/|" . $yesterday_pageviews . " pageviews>. Last " . $lastweek->format('l') . ", we had <https://www.google.com/analytics/web/?hl=en#report/visitors-overview/" . GOOGLE_ANALYTICS_WEB_ID . "/%3F_u.date00%3D" . $lastweek->format('Ymd') . "%26_u.date01%3D" . $lastweek->format('Ymd') . "%26overview-graphOptions.selected%3Danalytics.nthHour/|" . $lastweek_pageviews . " pageviews> (" . ($yesterday_pageviews-$lastweek_pageviews > 0 ? "+" : "" ) . ($yesterday_pageviews-$lastweek_pageviews) . ").";
